...
const auditKeys = require('./auditKeys.json');
...
const { CHROME_FLAGS, ONLY_CATEGORIES } = process.env;
...
exports.handler = async (event) => {
  ...
  const { url, webhook, testId, onlyChromeFlags, onlyCategories } = _.get(
    params,
    'cmd',
  );

  console.log(url, webhook, testId, onlyChromeFlags, onlyCategories);

  await checkJsonRequestForCustomProcessingOnLighthouse(
    onlyChromeFlags,
    onlyCategories,
  );

  const { artifacts, report, ...results } = await launchChromeAndRunLighthouse(
    url,
  );
  ...
  const payload = {
    ...
    results: {
      lhr: {
        // Shapes the payload, audits too large too send during local testing
        audits: auditKeys.reduce((acc, val) => {
          acc[val] = results.lhr.audits[val];
          return acc;
        }, {}),
      },
    },
...
async function checkJsonRequestForCustomProcessingOnLighthouse(
  onlyChromeFlags,
  onlyCategories,
  onlyAudits,
) {
